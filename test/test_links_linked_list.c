...
void free_link_list(Link *head) {
  ...
  while (head != NULL) {
    ...
    Link *next = head->next;
    free(head);
    // then when free the head then the head is set to next value that was saved
    // above
    head = next;
  }
}
...
int test1() {
  ...
  // then if test 1 is passed, then an appropiate message is displayed and it is
  // returned 1
...
int test3() {
  ...
  // next pointer of link1 points to link2, and next pointer of link2 points to
  // link3
  link1->next = link2;
  link2->next = link3;
