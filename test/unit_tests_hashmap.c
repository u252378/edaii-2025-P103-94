...
int insert_retrive_into_from_hashmap() {
  ...
  // allocate space for one document that will be inserted in the hashmap
  // created above
  ...
  // inserting document created for the test under the name "document_1" into
  // the hashmap created for the test
  ...
  // checking document inserted and retrieved is the same, and if so then the
...
int overwrite_key_hashmap() {
  ...
  if (document1 == NULL ||
      document2 == NULL) { // handling memory allocation fail
    printf("Memory allocation failed\n");
    if (document1 != NULL) {
      free(document1);
    }
  ...
  so then when inserting the document2 then the stored pointer for this key is
  updated to document2*/
  insertToHashMap(hashmap, "key", document1);
  insertToHashMap(hashmap, "key", document2);

  // then when retrieving the document under the name "key" then it should get
  // document2
...
int collision_chain() {
  ...
  if (doc1 == NULL || doc2 == NULL ||
      doc3 == NULL) { // handling memory allocation fail
    printf("Memory allocation failed\n");
    if (doc1 != NULL) {
      free(doc1);
    }
  ...
  */
  ...
  // then retrieve each document from hashmap to check that they were correctly
