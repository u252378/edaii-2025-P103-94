...
void add_vertex(Graph *graph, Document *document) {
  ...
  for (int j = 0; j < graph->num_vertices; j++) {
    ...
    graph->adjacency_matrix[j][graph->num_vertices] =
        0.0; // new column is initiliased with 0.0
  }
...
void delete_vertex(Graph *graph, int i) {
  ...
  for (int q = 0; q < graph->num_vertices - 1; q++) {
    ...
    graph->adjacency_matrix[q] = realloc(
        graph->adjacency_matrix[q], sizeof(float) * (graph->num_vertices - 1));
  }
  ...
  for (int i = 0; i < graph->num_vertices; i++) {
    for (int j = 0; j < graph->num_vertices; j++) {
      if (graph->adjacency_matrix[i][j] !=
          0.0) { // checking if there is an edge from vertex i to vertex j
