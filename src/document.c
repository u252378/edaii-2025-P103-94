...
Document *document_desserialize(char *path) {
  ...
  // parse id:
  while ((ch = fgetc(file)) != '\n') { // this will read characters until
                                       // newline
  ...
  doc->doc_id =
      atoi(buffer); // converts buffer to integer and store as document ID

  // parse title:
  bufferIdx = 0; // in order to prepare to read the next line (title) we reset
                 // the buffer index
  while ((ch = fgetc(file)) !=
  ...
  assert(bufferIdx < bufferSize);
  buffer[bufferIdx++] = '\0';
  doc->title = strdup(buffer); // same as in id, but it allocates memory and
    ...
    if (parsingLink) {        // parsing a link ID
      if (ch == ')') {        // end of link (ends with ")")
        ...
        linkBuffer[linkBufferIdx++] = ch; // store link character
               ...
               ']') { // found beginning of link id. ex: [my link text](123)
      parsingLink = true;
    }
  }
  ...
  char *body = (char *)malloc(sizeof(char) *
                              bufferIdx); // allocates memory for the body.
  strcpy(body,
         buffer); // copy the buffer content into the allocated body memory.
