...
ReverseIndex *reverseIndexInit(
    int slots) { // function to create the reverse index (the hash table)
  ReverseIndex *index =
...
void reverseIndexPut(ReverseIndex *index, char *word, DocumentsListNode *node) {
    ...
    ReverseIndexSlot *slotPtr = index->slots[slot];
    ...
    while (key) {
        if (strcmp(key->word, word) == 0) {
            // Verificar si el documento ya existe para esta palabra
            DocumentsListNode *current = key->values->head;
            while (current) {
                if (current->document->doc_id == node->document->doc_id) {
                    free(node); // Liberar el nodo duplicado
                    return;     // El documento ya está en la lista
                }
...
void reverseIndexSaveToFile(
    ReverseIndex *index,
    const char *filename) {          // save the word index to a text file
  ...
  for (int i = 0; i < index->slotsCount;
       i++) { // loop through each slot in the index
    ReverseIndexSlot *slot = index->slots[i];
    if (!slot)
    ...
    while (key) {                      // write the word followed by colon
      ...
      while (doc) {
        if (doc->document && doc->document->title) {
          fprintf(
              file, "%s,",
              doc->document->title); // write document title followed by comma
        }
        doc = doc->next;
      }
...
void reverseIndexLoadFromFile(
    ...
    Document *(*getDocByTitle)(
        const char *)) {             // load the word index from a text file
